...
export const verifySignatures = async (
  ...
  // step (1.5) - verify that the user has the name!
  let address = compositeSignatures[0]?.addr;
  let ownsProfile = await verifyOwnership(address, name);
  if (!ownsProfile) {
    console.error("You don't own this name..");
    return false;
  }
...
export function checkUserJWT(token) {
  try {
...
// returns whether the address owns the name
async function verifyOwnership(address, name) {
  let res = await getName(address);
  console.log("[Flow blockchain]", res);
  return res === name;
}
